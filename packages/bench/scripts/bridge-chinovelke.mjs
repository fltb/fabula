...
function sanitizeId(name) {
  ...
  return name.replace(/[^\u4e00-\u9fff_a-zA-Z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
}
...
const NOVEL_CHARACTER_INFO = {
  journey_to_west: {
    孙悟空: { gender: '男', role: 'protagonist', traits: ['神通广大', '桀骜不驯', '忠诚'], age_range: '500+' },
    唐僧:   { gender: '男', role: 'protagonist', traits: ['慈悲', '软弱', '执着'], age_range: '30+' },
    猪八戒: { gender: '男', role: 'supporting', traits: ['贪吃', '好色', '憨厚'], age_range: 'unknown' },
    沙僧:   { gender: '男', role: 'supporting', traits: ['忠厚', '勤劳', '沉默'], age_range: 'unknown' },
    白龙马: { gender: '男', role: 'supporting', traits: ['忠诚', '忍辱负重'], age_range: 'unknown' },
    观音菩萨: { gender: '女', role: 'supporting', traits: ['慈悲', '智慧', '法力无边'], age_range: 'unknown' },
    如来佛祖: { gender: '男', role: 'background', traits: ['至高无上', '法力无边'], age_range: 'unknown' },
    玉皇大帝: { gender: '男', role: 'antagonist', traits: ['威严', '官僚'], age_range: 'unknown' },
    太白金星: { gender: '男', role: 'background', traits: ['老练', '圆滑'], age_range: 'unknown' },
    二郎神:   { gender: '男', role: 'antagonist', traits: ['神通广大', '高傲'], age_range: 'unknown' },
    ...
    镇元大仙: { gender: '男', role: 'supporting', traits: ['神通广大', '重情义'], age_range: 'unknown' },
  },
  dream_of_red_chamber: {
    贾宝玉: { gender: '男', role: 'protagonist', traits: ['多情', '叛逆', '纯真'], age_range: '13-19' },
    林黛玉: { gender: '女', role: 'protagonist', traits: ['才华横溢', '敏感', '孤傲'], age_range: '12-17' },
    薛宝钗: { gender: '女', role: 'supporting', traits: ['端庄', '世故', '博学'], age_range: '14-19' },
    王熙凤: { gender: '女', role: 'supporting', traits: ['精明', '泼辣', '狠毒'], age_range: '20-25' },
    ...
    史湘云: { gender: '女', role: 'supporting', traits: ['豪爽', '才情', '乐观'], age_range: '13-18' },
    妙玉:   { gender: '女', role: 'supporting', traits: ['孤傲', '洁癖', '才情'], age_range: '18-20' },
    袭人:   { gender: '女', role: 'supporting', traits: ['温顺', '忠诚', '心机'], age_range: '16-20' },
    晴雯:   { gender: '女', role: 'supporting', traits: ['刚烈', '美丽', '直率'], age_range: '15-17' },
  },
  water_margin: {
    宋江:   { gender: '男', role: 'protagonist', traits: ['忠义', '矛盾', '领导力'], age_range: '30-40' },
    武松:   { gender: '男', role: 'protagonist', traits: ['勇猛', '正义', '嗜酒'], age_range: '25-30' },
    林冲:   { gender: '男', role: 'protagonist', traits: ['忍辱', '武艺高强', '悲剧'], age_range: '30-35' },
    鲁智深: { gender: '男', role: 'protagonist', traits: ['豪爽', '正义', '粗中有细'], age_range: '30-40' },
    李逵:   { gender: '男', role: 'supporting', traits: ['莽撞', '忠诚', '嗜杀'], age_range: '25-30' },
    吴用:   { gender: '男', role: 'supporting', traits: ['智谋', '阴险', '忠心'], age_range: '30-40' },
    卢俊义: { gender: '男', role: 'supporting', traits: ['武艺高强', '富有', '被逼上梁山'], age_range: '35-45' },
    高俅:   { gender: '男', role: 'antagonist', traits: ['奸诈', '弄权', '迫害忠良'], age_range: '40-60' },
    西门庆: { gender: '男', role: 'antagonist', traits: ['好色', '有钱', '霸道'], age_range: '25-35' },
...
function bridgeChiNovelKE(rawPath, outputPath) {
    ...
    if (annotations.location_hierarchy?.entries) {
      for (const entry of annotations.location_hierarchy.entries) {
        allLocations.push({
          ...
          name: entry.name,
          parent_id: entry.correct_parent ? sanitizeId(novelId + '_' + entry.correct_parent) : undefined,
          description: `${entry.name} — ${entry.tier}级别地点`,
          era: novel.genre === 'fantasy' ? '神话时代' : novel.genre === 'realistic' ? '清代' : '宋代',
        });
      }
    }
  }
  ...
  console.log(`Bridged ChiNovelKE: ${allCharacters.length} chars, ${allRelations.length} rels, ${allLocations.length} locs → ${outputPath}`);
}
