...
function resolveCredentialBase(env: Record<string, string | undefined>): string {
  ...
  throw new Error(
    'Cannot resolve credential base: set XDG_CONFIG_HOME or HOME',
  );
}
...
function resolveHostHome(env: Record<string, string | undefined>): string {
  ...
  throw new Error(
    'Cannot resolve Host home: set WORKBENCH_HOME, XDG_STATE_HOME, or HOME',
  );
}
...
function buildIdentityFromManifest(
  m: ArtifactManifestBuildIdentityV1,
): HostBuildIdentityV1 {
  return {
    version: 1,
...
export function validatePaths(paths: LaunchPaths): void {
  ...
  try {
    ...
    throw new LaunchDescriptorError(
      'HOST_ENTRY_NOT_FOUND', 'hostEntry',
  ...
  try {
    ...
    throw new LaunchDescriptorError(
      'WORKER_ENTRY_NOT_FOUND', 'workerEntry',
  ...
  try {
    ...
    throw new LaunchDescriptorError(
      'ASSETS_ROOT_NOT_FOUND', 'assetsRoot',
  ...
  if (!existsSync(hostHomeParent)) {
    throw new LaunchDescriptorError(
      'HOST_HOME_PARENT_NOT_FOUND', 'hostHome',
...
export function validateNodeVersion(
  ...
  if (actual !== required) {
    throw new LaunchDescriptorError(
      'NODE_VERSION_MISMATCH', 'nodeVersion',
