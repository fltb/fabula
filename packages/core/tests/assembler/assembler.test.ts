...
function scene(
  ...
  branchExistence: BranchSet = allBranch,
): SceneEntry {
  return { prose, metadata: {}, chapter, narrativeOrder, branchExistence };
}

describe('countNarrativeText', () => {
  it('counts words in plain text', () => expect(countNarrativeText('hello world', 'en')).toBe(2));
  it('ignores markdown headings', () =>
    expect(countNarrativeText('# Chapter 1\nSome text here.', 'en')).toBe(5));
  it('strips markdown links', () =>
    expect(countNarrativeText('See [this link](http://example.com) here.', 'en')).toBe(4));
  it('returns 0 for empty string', () => expect(countNarrativeText('', 'en')).toBe(0));
});
...
describe('filterScenesByBranchPath', () => {
  ...
  it('selects only scenes containing the requested path', () => {
    const pathA: BranchSet = {
      type: 'paths',
      paths: [{ decisions: [{ atEventId: 'E1', choiceId: 'a', narrativeOrder: 1 }] }],
    };
    const pathB: BranchSet = {
      type: 'paths',
      paths: [{ decisions: [{ atEventId: 'E1', choiceId: 'b', narrativeOrder: 1 }] }],
    };
...
describe('assembleNovel', () => {
  ...
  it('assembles ordered chapters from an immutable source snapshot', () => {
    const result = assembleNovel({
      source: source({
        scenes,
        discourseSequence: [
          { sceneId: 'E1a', sequence: 0, chapter: 1 },
          { sceneId: 'E1b', sequence: 1, chapter: 1 },
          { sceneId: 'E2a', sequence: 2, chapter: 2 },
        ],
        chapterTitles: new Map([
          [1, { chapter: 1, title: 'Opening', summary: '', intent: '', plannedScenes: 2 }],
          [2, { chapter: 2, title: 'Conclusion', summary: '', intent: '', plannedScenes: 1 }],
        ]),
  ...
  it('rejects empty and incomplete discourse sequences', () => {
    ...
    try {
      assembleNovel({ source: source() });
    } catch (error) {
      expect((error as AssemblyError).code).toBe(AssemblyErrorCode.NO_SCENES);
    }
    expect(() =>
      assembleNovel({
        source: source({ scenes: new Map([['E1', scene('text', 1, 1)]]), discourseSequence: [] }),
      }),
    ).toThrowError(/not in discourse sequence/);
  });
});
