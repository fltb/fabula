...
const source = (): ProjectSourceSnapshotV1 => {
  const entries: Record<string, string> = {
    'nova.yaml': 'project: release-assembly-debug\ntitle: Debug Test\nauthor: Tester\ndefaultModel: mock-pass2\n',
    ...
    'definitions/characters/alice.yaml': 'id: alice\nname: Alice\ntype: human\ndescription: Protagonist\ninitialState: {}\ntraits: []\n',
    ...
    'definitions/discourse-ledger.yaml': 'id: debug-test\nchapters:\n  - branch: main\n    chapter: 1\n    sceneIds: [E001]\nentries: []\n',
  };
...
function analysis(): AnalysisResult {
  const payload: Record<string, unknown> = { postconditions: { covered: [], dropped: [] }, preconditions: { violated: [] }, pov: { consistent: true, leaks: [] }, inventedDetails: [], quality: { proseScore: 8, maxScore: 10, strengths: ['clear'], weaknesses: [], estimatedWordCount: 80 }, threadProgressAchieved: [], foreshadowingDeployed: [], narrativeChecks: [], appearanceChecks: [], characterReferences: [], tenseDetected: 'past', conflictAnalysis: { primaryType: 'none', resolutionAchieved: true }, ruleChecks: [], knowledgeChecks: [], checklistResults: [] };
  return { eventId: 'E001', protocol: makeProtocol('Alice entered quietly.'), observations: makeObservations(payload, 'Alice entered quietly.'), analysis: payload };
}

it('debug renderNovel with explicit semantic runtime', async () => {
  const provider = new MockPass2Provider({ entries: { E001: { prose: 'Alice entered quietly.', analysis: analysis() } } });
  const result = await renderNovel({ version: 1, source: source(), selector: { type: 'all' }, mutation: { operationId: randomUUID(), actorId: 'renderer' }, model: 'mock-pass2' }, {
    provider,
