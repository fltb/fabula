...
} from '../ports/render-cache-repository.js';
...
function completeRecord(
  ...
  key: LayeredCacheKey,
  evidenceHash?: string,
): record is RenderCacheRecord {
  if (!record || typeof record !== 'object' || Array.isArray(record)) return false;
  const candidate = record as Partial<RenderCacheRecord> & { output?: Record<string, unknown> };
  if (
    candidate.version !== 1 ||
    !candidate.key ||
    candidate.key.version !== key.version ||
    candidate.key.sourceHash !== key.sourceHash
  )
    return false;
  if (!candidate.key.layers || canonicalJson(candidate.key.layers) !== canonicalJson(key.layers))
    return false;
  if (typeof candidate.recordHash !== 'string' || !/^[0-9a-f]{64}$/.test(candidate.recordHash))
    return false;
  if (!candidate.output || typeof candidate.output !== 'object' || Array.isArray(candidate.output))
    return false;
  if (typeof candidate.output.prose !== 'string' || candidate.output.prose.length === 0)
    return false;
...
export async function getCachedRender(
  ...
  try {
    ...
    if (record === null) {
      diagnostics?.push({ eventId, diagnosis: 'miss', detail: 'No cache record' });
      return null;
    }
    if (!completeRecord(record, lookup.key, lookup.evidenceHash)) {
      diagnostics?.push({
        eventId,
        diagnosis: 'corrupt',
        detail: 'Incomplete or mismatched cache record',
      });
      return null;
    }
...
export async function clearEventCache(
  ...
  try {
    ...
    /* best effort derived data */
  }
}

export async function clearRenderCache(
  repository: RenderCacheRepository,
  keys: readonly LayeredCacheKey[],
): Promise<void> {
...
export function verifyEvidenceChain(
  records: ReadonlyMap<string, RenderCacheRecord | null>,
  keys: ReadonlyMap<string, LayeredCacheKey>,
): VerifyChainResult {
  ...
  for (const [eventId, key] of keys) {
    ...
    result.totalCached++;
    if (completeRecord(record, key)) {
      result.valid++;
      result.details.push({ eventId, status: 'valid' });
    } else {
      result.stale++;
      result.details.push({ eventId, status: 'corrupt', reason: 'Invalid cache record' });
    }
  }
  return result;
}
