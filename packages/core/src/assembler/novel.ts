...
export function assembleNovel(options: AssembleOptions): AssembleResult {
  ...
  if (scenes.length === 0) throw new AssemblyError(AssemblyErrorCode.NO_SCENES, 'No scenes to assemble');
  ...
  for (const entry of source.discourseSequence) {
    ...
    if (!scene) {
      ...
      throw new AssemblyError(AssemblyErrorCode.MISSING_PROSE, `Scene "${eventId}" has no matching prose`);
    }
    if (seen.has(eventId)) throw new AssemblyError(AssemblyErrorCode.MISSING_PROSE, `Duplicate scene "${eventId}"`);
    seen.add(eventId);
    if (entry.chapter !== scene.chapter) throw new AssemblyError(AssemblyErrorCode.MISSING_PROSE, `Scene "${eventId}" has mismatched chapter`);
    orderedScenes.push(scene);
  }
  for (const scene of scenes) {
    if (!seen.has(scene.eventId)) throw new AssemblyError(AssemblyErrorCode.MISSING_PROSE, `Scene "${scene.eventId}" is not in discourse sequence`);
  }
  const chapterMetadata = new Map<number, ChapterMetadata>();
  for (const [chapter, metadata] of source.chapterTitles ?? []) chapterMetadata.set(chapter, metadata);
  const markdown = new ProseConcatenator().concatenate(orderedScenes, chapterMetadata, title ?? source.projectTitle);
  return {
    markdown,
    wordCount: orderedScenes.reduce((total, scene) => total + countWords(scene.prose, language), 0),
    sceneCount: orderedScenes.length,
    scenes: orderedScenes.map(({ eventId, chapter, narrativeOrder, branchExistence }) => ({ eventId, chapter, narrativeOrder, branchExistence })),
