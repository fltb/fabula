...
const requiredCodeCoverageSchema = z.object({
...
const thresholdsSchema = z.object({
  ...
  medianChecklistCoverage: z.number().min(0).max(1).default(0.90),
  minimumRunChecklistCoverage: z.number().min(0).max(1).default(0.80),
  maxMajorInventedDetails: z.number().min(0).default(0),
  medianWordCountAPE: z.number().min(0).max(1).default(0.25),
  maxSceneWordCountAPE: z.number().min(0).max(1).default(0.50),
  minimumChecklistJaccard: z.number().min(0).max(1).default(0.85),
  memorizationOverlap24: z.number().min(0).max(1).default(0.20),
...
export const closedLoopSpecSchema = z.object({
