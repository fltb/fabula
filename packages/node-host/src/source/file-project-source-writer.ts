...
import { SourceConflictError, SourcePathError, type FileProjectSourceLoader as FileProjectSourceLoaderContract, type FileProjectSourceWriter as FileProjectSourceWriterContract, type FileProjectSourceWriterOptions } from './types.js';
...
function validateLogicalPath(path: string): void {
  if (!path || path.includes('\\') || path.startsWith('/') || path.split('/').some((part) => part === '' || part === '..') || !approved(path)) {
