...
#### [x] STATE-2: 完整多元 Relationship 的状态规范

**完成备注（2026-07-21）**：实现 n-ary Relationship 类型系统（types/relationship.ts）：RelationshipTypeId/RelationshipTypeDefinition（roles + cardinality/mutability/continuityImpact）、三层 identity（RelationshipId/EpochId/MembershipId）、epoch lifecycle（active/suspended/dissolved）、5 种 dimension scope（global/role/member/subset/positional）、RelationshipTransaction。schemas/relationship.ts Zod。state/relationship-replay.ts 事务应用 + provider resolution + merge。replay.ts relationship 块替换为结构化事务应用（移除 relKey=participants.sort().join('_') + direction prose 解析）。二元关系 backward-compat。3 测试文件。验证：build 0，990/991 测试通过。
...
#### [x] STATE-3: 通用 Entity instance lifecycle 规范

**完成备注（2026-07-21）**：STATE-3a — EntityTypeCatalog/EntityDeclarationCatalog/AttributeDefinition/EntityRuntimeState 类型（types/entity-catalog.ts）；默认 catalog 覆盖 6 种 kind 全部硬编码属性（marital_status: mutable/lifecycle）；registry 重构为 catalog 驱动（移除 character 6-field 特殊处理 + rule 2-field 硬编码，defect #3 修复）；Entity 接口扩展 lifecycle + typeRef。STATE-3b — 12 个 validator 的 21 处硬编码属性检查全部替换为 catalog semanticRole/writePolicy 查询（defect #1 修复，zhu-fu marital_status 不再误报 world_rule）。STATE-3c — replay.ts lifecycle transactions（introduce/active/inactive/retired，retired 终态，same-storyTime 冲突检测）。3 个新测试文件/75 测试。验证：build 0，930/931 测试通过（1 个预存 ai-sdk 失败，无关）。
...
#### [x] STATE-4: 有限确定性 Knowledge/Belief 规范

**完成备注（2026-07-21）**：实现 Knowledge/Belief 类型系统（types/knowledge.ts）：PropositionCatalog（4 种：Grounded/Epistemic/Act/Intensional）、EpistemicLedger、ClaimSemanticState、ClaimEvidenceRecord、InformationAct（8 种）、GroupEpistemicQueryDefinition、CommonGroundRecord、NarrativeKnowledgeBoundary。知识 = 主体对 immutable proposition 的态度（非 copied Fact）。evaluate() deterministic three-valued。schemas/knowledge.ts Zod。state/knowledge-replay.ts claim/act 事务应用。validator/knowledge.ts 重构为 EpistemicLedger + catalog semanticRole。aggregator.ts getKnowledge stub 返回 EpistemicLedger。4 测试文件。验证：build 0，990/991 测试通过。
...
#### [x] STATE-5: Thread 的长程叙事结构规范

**完成备注（2026-07-21）**：实现 Thread 类型系统（types/thread.ts）：ThreadTypeCatalog/ThreadDeclarationCatalog（immutable/versioned）、ThreadId/ThreadRunId、ThreadRuntimeState（status/runId/phase/bindings/goalStates/milestoneStates/semanticStateHash）、ThreadTypeDefinition（roles/phases/lifecycle/reopen policy/time domain/goals/milestones）。Lifecycle：planned/active/blocked/completed/abandoned/retired。Canonical progress：goal（pending/active/achieved/failed/waived）+ milestone（pending/achieved/failed/waived/invalidated），无 scalar progress。Clock isolation：story vs discourse，禁止 cross-clock provider edges。ThreadTransaction。schemas/thread.ts Zod。state/thread-replay.ts 事务应用 + clock 兼容 + merge。replay.ts thread 块替换为结构化事务应用。validator/thread-progress.ts 重构为 ThreadRuntimeState。WorldState.threads 类型改为 Record<string,ThreadRuntimeState>。ThreadProgressEntry backward-compat。16 个消费者文件更新。3 测试文件/43 测试。验证：build 0，1033/1034 测试通过。
...
#### [x] STATE-6: Rule 的约束、审计与语义规范

**完成备注（2026-07-21）**：实现 Rule 类型系统（types/rule.ts）：RuleTypeDefinition/RuleSpecification/RuleId/RuleSpecificationId/RuleEpochId/RuleExceptionId、RuleRuntimeState（currentEpoch/activation: dormant|enabled|suspended|revoked/effectivenessLevel: full|limited|nullified/scopeBindings/exceptions/semanticHash）、RuleConstraint（4 种：state_invariant/transition_constraint/precondition_requirement/postcondition_requirement，enforcement: hard|audit|semantic，closed predicate AST）、RuleEvaluationRecord（compliant/violated/exempt）、RuleException（exempt/replaceWith，无 priority resolution）、RuleTransaction（8 种操作）。三种 enforcement channel：hard（commit 前拒绝）、audit（accept + immutable record）、semantic（Pass 2 only）。Fixed evaluation order per node。schemas/rule.ts Zod。state/rule-replay.ts 事务应用 + constraint evaluation + exception handling。replay.ts rule 块替换为结构化事务应用。validator/world-rule.ts 重构为 RuleRuntimeState。WorldState.rules 类型改为 Record<string,RuleRuntimeState>。RuleEffectEntry backward-compat（reinforce→enable, weaken→suspend, introduce_exception→add_exception, nullify→set_effectiveness:nullified）。4 测试文件/74 测试。验证：build 0，1083/1084 测试通过。
