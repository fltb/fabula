...
const source = (sourceHash: string): ProjectSourceSnapshotV1 => ({
  version: 1,
  documents: [],
  sourceHash,
});
const key = (sourceHash: string): LayeredCacheKey => ({
  version: 1,
  sourceHash,
  layers: { logical: 'l', surface: 's', validation: 'v', attempt: 'a' },
});
const record = (cacheKey: LayeredCacheKey): RenderCacheRecord => ({
  version: 1,
  key: cacheKey,
  recordHash: sha256Canonical(cacheKey),
  output: {
    prose: 'scene text',
    analysis: {
      eventId: 'cache-event',
...
class MemoryCache implements RenderCacheRepository {
  ...
  async get(input: { key: LayeredCacheKey }): Promise<RenderCacheRecord | null> {
    return this.values.get(sha256Canonical(input.key)) ?? null;
  }
  async put(input: { key: LayeredCacheKey; record: RenderCacheRecord }): Promise<void> {
    this.values.set(sha256Canonical(input.key), input.record);
  }
  async remove(input: { key: LayeredCacheKey }): Promise<void> {
    this.values.delete(sha256Canonical(input.key));
  }
}
...
describe('pure render cache identity', () => {
  it('canonicalizes object keys and evidence IDs only', () => {
    ...
    expect(
      computeEvidenceHash(
        'E0',
        [{ id: 'z', value: 'one' } as never],
        [{ id: 'a', value: 'two' } as never],
      ),
    ).toBe(
      computeEvidenceHash(
        'E0',
        [{ id: 'a', value: 'changed' } as never],
        [{ id: 'z', value: 'other' } as never],
      ),
    );
  });
  ...
  it('uses snapshot sourceHash and ignores provenance', () => {
    ...
    expect(
      buildLogicalKeyMaterial({
        ...
        sceneContractHash: 'c',
        worldStateHash: 'w',
        plannedDiscourseHash: 'd',
        branchDiscourseScopeHash: 'b',
        catalogVersionHashes: {},
        graphHash: 'g',
        styleProfileHash: 's',
        promptProviderId: 'p',
        language: 'en',
  ...
  it('treats partial and corrupt records as safe misses', async () => {
    ...
    await repository.put({
      key: cacheKey,
      record: { ...record(cacheKey), output: { prose: 'text' } },
    });
    expect(await getCachedRender(repository, { key: cacheKey })).toBeNull();
    await repository.put({
      key: cacheKey,
      record: { ...record(cacheKey), output: { prose: 'text', analysis: null } },
    });
    expect(await getCachedRender(repository, { key: cacheKey })).toBeNull();
  });
});
