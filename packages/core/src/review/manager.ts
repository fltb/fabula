...
export class ReviewManager {
  ...
  async readLedger(): Promise<ReviewLedgerSnapshot> {
    ...
    if (parsed.success) {
      ...
      return {
        ledger,
        contentHash: sha256Canonical(canonicalJsonValue(ledger)),
        legacy: false,
  ...
  async addReviewComment(
    ...
    await this.persist(
      { ...current.ledger, comments: [...current.ledger.comments, comment] },
      current.version,
    );
    return comment;
  }
  ...
  async getApplicableOpenComments(eventId: string, chapterNum: number): Promise<ReviewComment[]> {
    const comments = (await this.readLedger()).ledger.comments.filter(
      (c) =>
        c.status === 'open' &&
        (c.target.type === 'novel' ||
  ...
  async replaceReviewComment(
    ...
    const original = current.ledger.comments[index];
    ...
    const comments = [...current.ledger.comments];
    comments[index] = {
      ...original,
      status: 'superseded',
      resolvedAt: now,
    ...
    await this.persist({ ...current.ledger, comments }, current.version);
    return replacement;
  }
  ...
  async updateReviewComment(
    ...
    const comments: ReviewComment[] = [...current.ledger.comments];
  ...
  async applyComments(
    ...
    for (const id of ids)
      if (!current.ledger.comments.some((c) => c.id === id))
        throw new EditorialOperationError('INVALID_OPERATION', `Comment ${id} not found`);
    const comments: ReviewComment[] = current.ledger.comments.map((comment): ReviewComment => {
      if (!ids.includes(comment.id)) return comment;
      const updated: ReviewComment = {
        ...comment,
        applications: [...comment.applications, application],
        status: addressed.has(comment.id) ? 'addressed' : comment.status,
      };
      return reviewCommentSchema.parse(updated);
    });
    await this.persist({ ...current.ledger, comments }, current.version);
    return comments.filter((c) => ids.includes(c.id));
  }
  async getPatches(): Promise<ReviewPatch[]> {
    return (await this.readLedger()).ledger.patches;
  }
  async getSummary(): Promise<StatusSummary> {
    return getSummary((await this.readLedger()).ledger.comments, 0);
  }
  ...
  private assertHash(current: ReviewLedgerSnapshot, expected?: string) {
    if (expected !== undefined && current.contentHash !== expected)
      throw new EditorialOperationError(
        'STORAGE_CONFLICT',
        'Expected review ledger hash does not match current content',
      );
  }
  private async persist(ledger: ReviewLedgerV1, expectedVersion: number | null) {
    const value = canonicalJsonValue(toReviewLedger(ledger));
    const result = await this.execution.compareAndSwapReview({
      projectId: this.projectId,
      reviewId: key,
      expectedVersion,
      value: { version: 1, projectId: this.projectId, reviewId: key, value },
    });
    if (result.kind === 'conflict')
      throw new EditorialOperationError('STORAGE_CONFLICT', 'Review ledger version conflict');
  }
  private normalizeLegacy(raw: JsonObject): ReviewLedgerV1 {
    ...
    return toReviewLedger({ version: 1, comments, patches });
  }
}
