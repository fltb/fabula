...
function capabilityVersionLabel(
  owner: WorkbenchAdminOverviewV1['owner'],
): string {
  ...
  return (
    <div class="grid gap-[var(--wb-space-6)]" data-testid="admin-system-page">
      <header class="grid gap-[var(--wb-space-2)]">
        <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">System</p>
        <h2 class="font-display text-3xl tracking-[-0.025em] text-[var(--wb-ink)]">Host health &amp; source of truth</h2>
      ...
      <Show
        ...
        fallback={
          <section class={PANEL} aria-live="polite">
            <h3 class="text-base font-semibold text-[var(--wb-ink)]">System status is not loaded</h3>
      ...
      >
        {(overview) => (
          <>
            <section class="grid gap-[var(--wb-space-4)] md:grid-cols-2 xl:grid-cols-4" aria-label="Host health">
              <StatusCard label="Host" value={statusLabel(overview().hostStatus)} detail={formatDate(overview().generatedAt)} />
              <StatusCard
                label="Setup phase"
                value={statusLabel(overview().setup.phase)}
                detail={overview().setup.configurationPresent ? 'Configuration present' : 'Configuration not present'}
              />
              <StatusCard
              ...
              <StatusCard
                ...
                detail={overview().restartRequired ? 'Controlled restart required' : 'Listener policy active'}
                tone={overview().restartRequired ? 'warning' : 'ready'}
              />
            </section>

            <section class={PANEL} aria-labelledby="admin-config-source-heading" data-testid="config-source-panel">
              <div class="flex flex-wrap items-start justify-between gap-[var(--wb-space-4)]">
                <div>
                  <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">Configuration source</p>
                  <h3 id="admin-config-source-heading" class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]">
                    Safe source status
                  </h3>
                </div>
            ...
            <section class="grid gap-[var(--wb-space-6)] lg:grid-cols-2">
              <section class={PANEL} aria-labelledby="admin-runtime-heading">
                <div class="flex items-start justify-between gap-[var(--wb-space-3)]">
                  <div>
                    <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">Runtime</p>
                    <h3 id="admin-runtime-heading" class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]">
                ...
                <dl class="mt-[var(--wb-space-5)] grid gap-[var(--wb-space-4)] sm:grid-cols-2">
                  ...
                  <SafeField label="Capability version" value={capabilityVersionLabel(overview().owner)} />
                  <SafeField label="Default project" value={overview().setup.defaultProjectId ?? 'Not selected'} />
                  <SafeField label="Network mode" value={overview().setup.network.mode} />
                </dl>
              </section>

              <section class={PANEL} aria-labelledby="admin-template-heading">
                <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">Transparent configuration</p>
                <h3 id="admin-template-heading" class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]">
                ...
                <div class="mt-[var(--wb-space-4)] flex flex-wrap gap-[var(--wb-space-3)]">
                  ...
                  <button class={SECONDARY_BUTTON} type="button" onClick={() => void copyTemplate()}>
                    Copy template
                  </button>
                </div>
                <p class="mt-[var(--wb-space-3)] min-h-[1.25rem] text-xs text-[var(--wb-success)]" aria-live="polite">
                  {templateStatus()}
                </p>
              </section>
            </section>
            ...
            <Show when={props.operations?.configurationOperations.length}>
              <section class={PANEL} aria-labelledby="admin-recent-config-heading">
                <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">Recent activity</p>
                <h3 id="admin-recent-config-heading" class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]">
                  ...
                  <For each={props.operations?.configurationOperations.slice(0, 3) ?? []}>
                    {(operation) => (
                      <li class="flex flex-wrap items-baseline justify-between gap-[var(--wb-space-3)] border-t border-[var(--wb-border)] pt-[var(--wb-space-3)] text-sm">
                        <span class="font-semibold text-[var(--wb-ink)]">{operation.origin} · {operation.status}</span>
                        <span class="text-xs text-[var(--wb-muted)]">{formatDate(operation.at)}</span>
                      </li>
                    )}
                  </For>
                </ul>
...
function StatusCard(props: { readonly label: string; readonly value: string; readonly detail: string; readonly tone?: 'ready' | 'warning' }) {
  return (
    <article class={`${PANEL} grid gap-[var(--wb-space-2)]`}>
      <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">{props.label}</p>
      <p class={`text-2xl font-semibold ${props.tone === 'warning' ? 'text-[var(--wb-warning)]' : 'text-[var(--wb-ink)]'}`}>
        {props.value}
      </p>
...
function SafeField(props: { readonly label: string; readonly value: string }) {
  return (
    <div class="grid gap-[var(--wb-space-1)]">
      <dt class="text-xs font-bold uppercase tracking-[0.06em] text-[var(--wb-muted)]">{props.label}</dt>
