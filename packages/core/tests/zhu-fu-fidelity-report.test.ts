...
const ORIGINAL = fs.readFileSync(path.join(REPO_ROOT, 'fixtures', 'zhu-fu', 'reference', 'original.txt'), 'utf-8');
...
function setupFixture(): { fixtureDir: string; candidateDir: string; reportPath: string } {
  ...
  for (const [eventId, [start, end]] of Object.entries(EVENT_LINES)) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ prose: ORIGINAL.split('\n').slice(start - 1, end).join('\n'), released: true, metadata: { attempts: 1 } }),
    );
  }

  return { fixtureDir, candidateDir, reportPath: path.join(fixtureDir, 'fidelity.md') };
}

describe('zhu-fu-fidelity-report', () => {
  it('scores exact original scene spans at 100% and writes a paired JSON report', () => {
    const { fixtureDir, candidateDir, reportPath } = setupFixture();
    try {
      const stdout = execFileSync(
        'node',
        [SCRIPT, fixtureDir, '--render-dir', candidateDir, '--label', 'exact', '--output', reportPath],
      ...
      const metrics = JSON.parse(fs.readFileSync(path.join(fixtureDir, 'fidelity.json'), 'utf-8')) as {
      ...
      expect(metrics.events.every((event) => event.directScore === 1 && event.bigramF1 === 1)).toBe(true);
    } finally {
