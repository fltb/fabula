...
import { CapabilityGateError, CapabilityRegistry } from '../../src/state/capability-manifest.ts';
import type { CapabilityManifest, CapabilityManifestEntry } from '../../src/types/index.ts';

// ─── Helpers ─────────────────────────────────────────────────────────────────

function makeEntry(
  overrides: Partial<CapabilityManifestEntry> & { capabilityId: string },
...
describe('CapabilityRegistry — manifest registration', () => {
  it('registers entries from a manifest', () => {
    ...
    reg.loadManifest(makeManifest([makeEntry({ capabilityId: 'state_replay', status: 'S' })]));
...
describe('CapabilityRegistry — missing entry rejection', () => {
  it('assertEntry throws for missing capabilityId', () => {
    const reg = new CapabilityRegistry();
    reg.loadManifest(makeManifest([makeEntry({ capabilityId: 'state_replay' })]));
  ...
  it('assertInputCovered throws for uncovered input form', () => {
    const reg = new CapabilityRegistry();
    reg.loadManifest(
      makeManifest([makeEntry({ capabilityId: 'state_replay', supportedInputForms: ['yaml'] })]),
    );
...
describe('CapabilityRegistry — stage 1 gate', () => {
  it('passes when S core capabilities are manifest-complete', () => {
    const reg = new CapabilityRegistry();
    reg.loadManifest(
      makeManifest([makeEntry({ capabilityId: 'state_replay', status: 'S', stageGate: 1 })]),
    );
...
describe('CapabilityRegistry — validateAllStages', () => {
    ...
    reg.loadManifest(
      makeManifest([
        ...
        makeEntry({
          ...
          status: 'C',
          stageGate: 2,
          provenanceRequirements: ['annotation'],
          evidenceArtifactHash: 'hash',
        }),
  ...
  it('fails on stage 1 when S capability is incomplete', () => {
    ...
    reg.loadManifest(
      makeManifest([
        makeEntry({
          capabilityId: 'surface_scheduler_contract',
          status: 'S',
          stageGate: 1,
          referenceCaseIds: [],
        }),
