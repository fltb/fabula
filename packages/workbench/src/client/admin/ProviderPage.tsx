...
export function ProviderPage(props: ProviderPageProps) {
  ...
  const saveCredential = () => {
    void run(async () => {
      ...
      if (!response) throw new Error('The owner client is unavailable.');
      setProvider((current) =>
        current ? { ...current, configured: response.configured } : current,
      );
    ...
    void run(async () => {
      ...
      setMessage(
        response.validation === 'valid'
          ...
          : `Provider validation failed: ${response.code ?? 'PROVIDER_VALIDATION_FAILED'}.`,
      );
    });
  };
  ...
  const clearCredential = () => {
    ...
    void run(async () => {
      ...
      setProvider((current) =>
        current
          ? { ...current, configured: response.configured, lastValidation: 'unvalidated' }
          : current,
      );
  ...
  return (
    <div class="grid gap-[var(--wb-space-6)]" data-testid="admin-provider-page">
      ...
      <Show
        ...
        fallback={
          <section class={PANEL}>
            <h3 class="text-base font-semibold text-[var(--wb-ink)]">Provider is not configured</h3>
            <p class="mt-[var(--wb-space-2)] text-sm leading-6 text-[var(--wb-muted)]">
              Save endpoint/model settings to begin provider setup.
            </p>
          </section>
        }
      >
        {(current) => (
          <section class={PANEL} aria-labelledby="provider-status-heading">
            <div class="flex flex-wrap items-start justify-between gap-[var(--wb-space-3)]">
              ...
              <span
                class={`rounded-full px-[var(--wb-space-3)] py-[var(--wb-space-2)] text-xs font-bold ${current().configured ? 'bg-[var(--wb-ready-surface)] text-[var(--wb-success)]' : 'bg-[var(--wb-loading-surface)] text-[var(--wb-warning)]'}`}
              >
      ...
      <section class="grid gap-[var(--wb-space-6)] lg:grid-cols-2">
        <section class={PANEL} aria-labelledby="provider-settings-heading">
          ...
          >
            <Field label="New base URL" id="provider-base-url">
              <input
            ...
            <Field label="New model" id="provider-model">
              <input
                id="provider-model"
                class={INPUT}
                value={model()}
        ...
        <section class={PANEL} aria-labelledby="provider-credential-heading">
          ...
          <h3
            id="provider-credential-heading"
            class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]"
          >
            Store provider credential
          </h3>
          <p class="mt-[var(--wb-space-2)] text-xs leading-5 text-[var(--wb-muted)]">
            The input is sent once to the Host credential store, then cleared from this component.
            It is never included in YAML, operations, or preferences.
          </p>
          <form
            class="mt-[var(--wb-space-5)] grid gap-[var(--wb-space-4)]"
            onSubmit={(event) => {
          ...
          >
            <Field label="Provider API key" id="provider-api-key">
              <input
                id="provider-api-key"
                class={INPUT}
                type="password"
            ...
            <div class="flex flex-wrap gap-[var(--wb-space-3)]">
              <button class={BUTTON} type="submit" disabled={!authorized() || busy() || !apiKey()}>
              ...
              <button
                ...
                disabled={!authorized() || busy() || !provider()?.configured}
              >
                Clear credential
              </button>
            </div>
        ...
        <AlertDialog.Portal>
          ...
          <AlertDialog.Content class="fixed left-1/2 top-1/2 z-50 grid w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 gap-[var(--wb-space-4)] rounded-[var(--wb-radius-lg)] border border-[var(--wb-border-strong)] bg-[var(--wb-surface)] p-[var(--wb-space-6)] shadow-[var(--wb-shadow-drawer)]">
            <AlertDialog.Title class="font-display text-xl text-[var(--wb-ink)]">
              Clear stored provider credential?
            </AlertDialog.Title>
            <AlertDialog.Description class="text-sm leading-6 text-[var(--wb-muted)]">
              Agent/provider actions will remain unavailable until a new credential is stored. The
              Host will not delete any project source.
            ...
            <div class="flex justify-end gap-[var(--wb-space-3)]">
              ...
              <button class={DANGER_BUTTON} type="button" onClick={clearCredential}>
                Clear credential
              </button>
            </div>
          </AlertDialog.Content>
