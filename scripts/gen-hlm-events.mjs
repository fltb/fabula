...
const manifest = JSON.parse(readFileSync(
  join(REPO, 'bench-data/corpus/dream-of-red-chamber/source-manifest.json'), 'utf-8'));
const src = readFileSync(
  join(REPO, 'bench-data/corpus/dream-of-red-chamber/source.txt'), 'utf-8');
...
for (let i = 0; i < targetChapters.length; i++) {
  ...
  const ch = manifest.chapters.find(c => c.title.includes(`第${chNum}回`)) || manifest.chapters[chNum-1];
  if (!ch) { console.log(`Skip ch${chNum}: not found`); continue; }
...
6. 输出必须是合法YAML，不要有任何markdown标记`;
  ...
  for (let attempt = 0; attempt < 3 && !success; attempt++) {
    try {
      ...
      let yamlStr = result.replace(/^```ya?ml\n?/gm, '').replace(/```$/gm, '').trim();
      ...
      // Write file
      const slug = parsed.title ? parsed.title.replace(/[^a-zA-Z0-9\u4e00-\u9fff]/g, '_').slice(0,40) : `ch${chNum}`;
