...
const snapshot = (documents = [document('definitions/a.yaml', 'a'), document('definitions/b.yaml', 'b')]) => ({
  version: 1 as const,
  documents,
  sourceHash: hash,
});

const jsonRoundTrip = <T>(schema: { parse(value: unknown): T }, value: T): T => schema.parse(JSON.parse(JSON.stringify(value)));
...
describe('pure Core contract schemas', () => {
  it('round-trips a source snapshot and preserves sorted logical documents', () => {
    ...
    expect(jsonRoundTrip(sourceParseResultV1Schema, value.documents[0].parseResult)).toEqual(value.documents[0].parseResult);
  });
  ...
  it('round-trips source deletion changes and source analysis', () => {
    ...
    const analysis = {
      version: 1 as const,
      current: snapshot(),
      candidate: snapshot([document('definitions/a.yaml', 'changed'), document('definitions/b.yaml', 'b')]),
      changes: [deletion],
      affectedEventIds: ['event-1'],
      diagnostics: [{ code: 'notice', severity: 'info' as const, message: 'checked', logicalPath: null }],
    };
  ...
  it('round-trips a complete cache record and rejects a key-record version mismatch', () => {
    const key = { version: 1 as const, sourceHash: hash, layers: { graph: 'graph-v1', prompt: 'prompt-v1' } };
    ...
    const record = { version: 1 as const, key, recordHash: otherHash, output: { prose: 'A quiet room.', analysis } };
    expect(jsonRoundTrip(renderCacheRecordSchema, record)).toEqual(record);
    expect(renderCacheRecordSchema.safeParse({ ...record, output: { ...record.output, analysis: { eventId: 'event-1' } } }).success).toBe(false);
    expect(renderCacheRecordSchema.safeParse({ ...record, key: { ...key, version: 2 } }).success).toBe(false);
  });

  it('round-trips state event append and snapshot records', () => {
    const key = { projectId: 'project-1', streamId: 'world', branchId: 'main' };
    const events = [
      { eventId: 'event-1', sequence: 0, type: 'fact.set', payload: { entity: 'hero', attribute: 'mood', value: 'calm' } },
      { eventId: 'event-2', sequence: 1, type: 'fact.set', payload: { entity: 'hero', attribute: 'place', value: 'room' } },
    ];
    ...
    const snapshotRecord = { version: 1 as const, key, schema: 'world-state', schemaVersion: 1, sequence: 1, state: { hero: { mood: 'calm' } }, snapshotHash: hash };
    expect(jsonRoundTrip(stateSnapshotRecordSchema, snapshotRecord)).toEqual(snapshotRecord);

    expect(stateAppendSuccessSchema.safeParse({ ...append, events: [events[1], events[0]] }).success).toBe(false);
    expect(stateAppendSuccessSchema.safeParse({ ...append, events: [events[0], { ...events[1], eventId: events[0].eventId, sequence: 1 }] }).success).toBe(false);
  });

  it('rejects unsafe paths, unordered documents, and invalid deletion pairings', () => {
    expect(projectSourceSnapshotV1Schema.safeParse({ ...snapshot(), documents: [document('../escape.yaml', 'x'), document('definitions/b.yaml', 'b')] }).success).toBe(false);
    expect(projectSourceSnapshotV1Schema.safeParse({ ...snapshot(), documents: [document('definitions/b.yaml', 'b'), document('definitions/a.yaml', 'a')] }).success).toBe(false);
    expect(sourceChangeV1Schema.safeParse({ logicalPath: 'definitions/removed.yaml', beforeContent: 'old', beforeHash: null, afterContent: null, afterHash: null }).success).toBe(false);
    expect(sourceChangeV1Schema.safeParse({ logicalPath: 'definitions/new.yaml', beforeContent: null, beforeHash: null, afterContent: 'new', afterHash: null }).success).toBe(false);
  });
});
