import { realpathSync, lstatSync, readdirSync, readFileSync } from 'node:fs';
...
import { SourcePathError, type FileProjectSourceLoader as FileProjectSourceLoaderContract, type FileProjectSourceLoaderOptions } from './types.js';

const ENTITY_DIRS = ['characters', 'locations', 'items', 'factions', 'relationships', 'rules', 'narrators', 'assertions'];
...
function assertContained(root: string, target: string): void {
  ...
  if (rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel)) throw new SourcePathError(`Path escapes project root: ${target}`);
}
...
export class FileProjectSourceLoader implements FileProjectSourceLoaderContract {
  ...
  constructor(options: FileProjectSourceLoaderOptions = {}) { this.parse = options.parse ?? ((content) => YAML.parse(content)); }
  ...
  load(projectRoot: string): ProjectSourceSnapshotV1 {
    ...
    const documents: SourceDocumentV1[] = [...paths].filter((p) => this.existsFile(root, p)).sort(compareLogicalPaths).map((p) => this.document(root, p));
    return { version: 1, documents, sourceHash: computeSourceHash(documents) };
  }

  private existsFile(root: string, logical: string): boolean {
    const target = join(root, ...logical.split('/')); assertContained(root, target);
    try { const stat = lstatSync(target); if (stat.isSymbolicLink()) throw new SourcePathError(`Symlink source path rejected: ${logical}`); return stat.isFile(); } catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return false; throw error; }
  }
  private collect(root: string, logicalDir: string, paths: Set<string>): void {
    const target = join(root, ...logicalDir.split('/')); assertContained(root, target);
    let entries; try { entries = readdirSync(target, { withFileTypes: true }); } catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return; throw error; }
    for (const entry of entries.sort((a, b) => compareLogicalPaths(a.name, b.name))) {
      const child = `${logicalDir}/${entry.name}`; assertLogicalPath(child);
      if (entry.isSymbolicLink()) throw new SourcePathError(`Symlink source path rejected: ${child}`);
  ...
  private collectChapters(root: string, paths: Set<string>): void {
    ...
    let entries; try { entries = readdirSync(base, { withFileTypes: true }); } catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return; throw error; }
    for (const entry of entries.sort((a, b) => compareLogicalPaths(a.name, b.name))) {
      if (entry.isSymbolicLink()) throw new SourcePathError(`Symlink source path rejected: chapters/${entry.name}`);
      if (!entry.isDirectory() || !/^chapter_[0-9]{2}$/.test(entry.name)) continue;
      const dir = `chapters/${entry.name}`; const target = join(base, entry.name);
      for (const file of readdirSync(target, { withFileTypes: true }).sort((a, b) => compareLogicalPaths(a.name, b.name))) {
        const path = `${dir}/${file.name}`; assertLogicalPath(path);
        if (file.isSymbolicLink()) throw new SourcePathError(`Symlink source path rejected: ${path}`);
        if (file.isFile() && (/^_chapter\.yaml$/.test(file.name) || /^E[^/]+\.yaml$/.test(file.name))) paths.add(path);
      }
    }
  }
