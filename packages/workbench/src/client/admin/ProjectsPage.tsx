...
function receiptText(receipt: { status: string; activeRevision: string | null; candidateRevision: string | null } | undefined): string {
  if (!receipt) return '';
...
export function ProjectsPage(props: ProjectsPageProps) {
  ...
  const projectOptions = () => projects().map(({ projectId: id, displayName: name }) => ({ projectId: id, displayName: name }));
  ...
  const validateProject = () => {
    ...
    void run(async () => {
      ...
      setMessage(result.validation === 'valid' ? 'The Host validated the project.' : `Validation failed: ${result.code ?? 'PROJECT_INVALID_ROOT'}.`);
    });
  };
  ...
  const saveProject = () => {
    void run(async () => {
      ...
      if (response.project) {
        setProjects((current) => {
          const without = current.filter((project) => project.projectId !== response.project?.projectId);
  ...
  const openSelected = () => {
    ...
    void run(async () => {
      ...
      if (!response) throw new Error('The owner client is unavailable.');
      setProjects((current) => current.map((project) => project.projectId === selected.projectId ? { ...project, open: true } : project));
  ...
  const closeSelected = () => {
    ...
    void run(async () => {
      ...
      if (!response) throw new Error('The owner client is unavailable.');
      setProjects((current) => current.map((project) => project.projectId === selected.projectId ? { ...project, open: false } : project));
  ...
  return (
    <div class="grid gap-[var(--wb-space-6)]" data-testid="admin-projects-page">
      <header class="grid gap-[var(--wb-space-2)]">
        <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">Projects</p>
        <h2 class="font-display text-3xl tracking-[-0.025em] text-[var(--wb-ink)]">Project registry</h2>
      ...
      <section class={PANEL} aria-labelledby="project-form-heading">
        <div class="flex flex-wrap items-start justify-between gap-[var(--wb-space-3)]">
          <div>
            <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">Owner action</p>
            <h3 id="project-form-heading" class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]">Validate or register a project</h3>
          </div>
        ...
        <form class="mt-[var(--wb-space-5)] grid gap-[var(--wb-space-4)]" onSubmit={(event) => { event.preventDefault(); saveProject(); }}>
          <div class="grid gap-[var(--wb-space-4)] md:grid-cols-2">
            <Field label="Project id" hint="Stable identifier, not a filesystem path.">
              <input class={INPUT} value={projectId()} onInput={(event) => setProjectId(event.currentTarget.value)} autocomplete="off" disabled={!authorized() || busy()} />
            </Field>
            <Field label="Display name">
              <input class={INPUT} value={displayName()} onInput={(event) => setDisplayName(event.currentTarget.value)} autocomplete="off" disabled={!authorized() || busy()} />
            </Field>
          </div>
          <Field label="Project root" hint="Consumed by the Host for validation; never echoed after the request.">
            <input class={INPUT} type="text" value={root()} onInput={(event) => setRoot(event.currentTarget.value)} autocomplete="off" spellcheck={false} disabled={!authorized() || busy()} />
          </Field>
          <div class="flex flex-wrap items-center gap-[var(--wb-space-3)]">
            <button class={SECONDARY_BUTTON} type="button" onClick={validateProject} disabled={!authorized() || busy()}>
            ...
            <span class={`text-sm ${validation() === 'invalid' ? 'text-[var(--wb-danger)]' : 'text-[var(--wb-muted)]'}`} aria-live="polite">
              {validation() === 'valid' ? 'Validated' : validation() === 'invalid' ? 'Invalid' : ''}
            </span>
          </div>
        </form>
        <Show when={message()}>
          <p class="mt-[var(--wb-space-3)] text-sm text-[var(--wb-success)]" aria-live="polite">{message()}</p>
        </Show>
        <Show when={error()}>
          <p class="mt-[var(--wb-space-3)] text-sm text-[var(--wb-danger)]" role="alert">{error()}</p>
        </Show>
      </section>

      <Show when={projects().length > 0} fallback={<section class={PANEL}><h3 class="text-base font-semibold text-[var(--wb-ink)]">No projects registered</h3><p class="mt-[var(--wb-space-2)] text-sm leading-6 text-[var(--wb-muted)]">Validate a project root above to begin the Host setup.</p></section>}>
        <section class="grid gap-[var(--wb-space-4)]" aria-labelledby="project-list-heading">
          <div class="flex flex-wrap items-end justify-between gap-[var(--wb-space-4)]">
            <div>
              <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">Safe registry</p>
              <h3 id="project-list-heading" class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]">Registered projects</h3>
            </div>
            <div class="min-w-[16rem]">
              <Combobox<ProjectOption>
                ...
                value={projectOptions().find((project) => project.projectId === selectedProjectId()) ?? null}
              ...
              >
                <Combobox.Label class="mb-[var(--wb-space-1)] block text-xs font-bold uppercase tracking-[0.06em] text-[var(--wb-muted)]">Runtime selection</Combobox.Label>
          ...
          <Show when={selectedProject()}>
            {(selected) => (
              <div class={`${PANEL} flex flex-wrap items-center justify-between gap-[var(--wb-space-3)]`}>
                <div>
                  <p class="font-semibold text-[var(--wb-ink)]">{selected().displayName}</p>
                  <p class="mt-1 text-xs text-[var(--wb-muted)]">{selected().open ? 'Runtime open' : 'Runtime closed'}</p>
                </div>
                <div class="flex flex-wrap gap-[var(--wb-space-2)]">
                  <button class={SECONDARY_BUTTON} type="button" onClick={openSelected} disabled={!authorized() || busy() || selected().open}>Open runtime</button>
                  <button class={SECONDARY_BUTTON} type="button" onClick={closeSelected} disabled={!authorized() || busy() || !selected().open}>Close runtime</button>
                </div>
              </div>
            )}
          </Show>
          <div class="grid gap-[var(--wb-space-3)]">
            <For each={projects()}>
              {(project) => (
                <article class={`${PANEL} grid gap-[var(--wb-space-4)] md:grid-cols-[minmax(0,1fr)_auto]`}>
                  <div class="grid gap-[var(--wb-space-2)]">
                    <div class="flex flex-wrap items-center gap-[var(--wb-space-3)]">
                      <h4 class="font-semibold text-[var(--wb-ink)]">{project.displayName}</h4>
                      <code class="rounded bg-[var(--wb-surface-muted)] px-2 py-1 text-xs text-[var(--wb-ink-soft)]">{project.projectId}</code>
                    </div>
                    <div class="flex flex-wrap gap-[var(--wb-space-2)] text-xs">
                      <span class="rounded-full bg-[var(--wb-ready-surface)] px-[var(--wb-space-2)] py-1 text-[var(--wb-success)]">validation: {project.validation}</span>
                      <span class="rounded-full bg-[var(--wb-surface-muted)] px-[var(--wb-space-2)] py-1 text-[var(--wb-muted)]">{project.open ? 'open' : 'closed'}</span>
                      <Show when={project.defaultProject}><span class="rounded-full bg-[var(--wb-accent-wash)] px-[var(--wb-space-2)] py-1 text-[var(--wb-accent-deep)]">default</span></Show>
                    </div>
                  </div>
                  <div class="flex items-center justify-end">
                    <button class={DANGER_BUTTON} type="button" onClick={() => setConfirmProjectId(project.projectId)} disabled={!authorized() || busy()}>
                      Remove project
                    </button>
                  </div>
                </article>
      ...
      <AlertDialog open={confirmProjectId() !== null} onOpenChange={(open) => { if (!open) setConfirmProjectId(null); }}>
        <AlertDialog.Portal>
          ...
          <AlertDialog.Content class="fixed left-1/2 top-1/2 z-50 grid w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 gap-[var(--wb-space-4)] rounded-[var(--wb-radius-lg)] border border-[var(--wb-border-strong)] bg-[var(--wb-surface)] p-[var(--wb-space-6)] shadow-[var(--wb-shadow-drawer)]">
            <AlertDialog.Title class="font-display text-xl text-[var(--wb-ink)]">Remove project from the registry?</AlertDialog.Title>
            <AlertDialog.Description class="text-sm leading-6 text-[var(--wb-muted)]">
              This closes a runtime if necessary and removes only the Host registry entry. The project directory is not deleted and the action may be recovered by registering it again.
            ...
            <div class="flex justify-end gap-[var(--wb-space-3)]">
              ...
              <button class={DANGER_BUTTON} type="button" onClick={() => { const id = confirmProjectId(); if (id) removeProject(id); }}>Remove project</button>
            </div>
          </AlertDialog.Content>
        </AlertDialog.Portal>
      </AlertDialog>
...
function Field(props: { readonly label: string; readonly hint?: string; readonly children: JSX.Element }) {
  return (
    <label class="grid gap-[var(--wb-space-2)] text-sm font-semibold text-[var(--wb-ink-soft)]">
      <span>{props.label}</span>
      <Show when={props.hint}><span class="text-xs font-normal leading-5 text-[var(--wb-muted)]">{props.hint}</span></Show>
      {props.children}
    </label>
  );
}
