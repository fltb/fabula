...
const scene = (prose: string, revisionId: string, source = sourceHash) => ({
  version: 1 as const,
  projectId: 'manual-edit',
  eventId: 'E001',
  sourceHash: source,
  revisionId,
  prose,
  proseHash: hash(prose),
  sceneHash: hash(`${source}\0${prose}`),
});
...
describe('manual edit lock freshness — semantic execution repository', () => {
  it('accepts a scene head and rejects stale compare-and-swap writes', async () => {
    ...
    const committed = await repository.compareAndSwapAcceptedScene({
      projectId: original.projectId,
      eventId: original.eventId,
      expectedVersion: 1,
      value: replacement,
    ...
    expect(
      (
        await repository.resolveAcceptedArtifact({
          projectId: original.projectId,
          eventId: original.eventId,
        })
      )?.prose,
    ).toBe(replacement.prose);
  });
  ...
  it('marks a locked head stale when the accepted source hash changes', async () => {
    ...
    expect(
      (
        await repository.readAcceptedScene({
          projectId: original.projectId,
          eventId: original.eventId,
        })
      )?.value.sourceHash,
  ...
  it('preserves revision lineage while changing prose', async () => {
    ...
    const revision = {
      ...
      sourceHash,
      value: { prose: 'New accepted prose.', restoredFromRevisionId: original.revisionId },
    };
    const stored = await repository.compareAndSwapSceneRevision({
      projectId: original.projectId,
      eventId: original.eventId,
    ...
    const restored = await repository.readSceneRevision({
      projectId: original.projectId,
      eventId: original.eventId,
      revisionId: revision.revisionId,
    });
    expect(restored?.value.parentRevisionId).toBe(original.revisionId);
    expect(restored?.value.value).toEqual({
      prose: 'New accepted prose.',
      restoredFromRevisionId: original.revisionId,
    });
  });
});
