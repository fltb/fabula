...
describe('strict YAML compiler', () => {
  it('preserves valid typed YAML and rejects malformed, unknown, and absent required files', () => {
    const source = snapshot({ 'event.yaml': event, 'malformed.yaml': 'event: [', 'unknown.yaml': `${event}unsupported: true\n` });
    expect(readYamlFile({ logicalPath: 'event.yaml', snapshot: source, schema: eventFileSchema })?.event).toBe('E1');
    for (const logicalPath of ['malformed.yaml', 'unknown.yaml', 'missing.yaml']) {
      try { readYamlFile({ logicalPath, snapshot: source, schema: eventFileSchema }); throw new Error('expected ConfigError'); }
      catch (error) { expect(error).toBeInstanceOf(ConfigError); expect((error as ConfigError).code).toBe('CONFIG_INVALID'); expect((error as ConfigError).context.path).toContain(logicalPath); }
    }
  });

  it('maps structured nested YAML timestamps and preserves the story AST for fact validity', () => {
    const source = snapshot({ 'structured.yaml': structuredEvent, 'indeterminate-anchor.yaml': indeterminateAnchor });
    const parsed = readYamlFile({ logicalPath: 'structured.yaml', schema: eventFileSchema, snapshot: source });
    const mapped = new EntityMapper(source).mapToNarrativeEvent(parsed!);
    expect(mapped.storyTime).toEqual({ type: 'relative', anchor: 'arrival', offset: { amount: 90, unit: 'minute' } });
    ...
    try { readYamlFile({ logicalPath: 'indeterminate-anchor.yaml', schema: worldInitialStateSchema, snapshot: source }); throw new Error('expected ConfigError'); }
    catch (error) { expect(error).toBeInstanceOf(ConfigError); expect((error as ConfigError).context.path).toContain('indeterminate-anchor.yaml'); expect((error as ConfigError).context.path).toContain('timeAnchors.0.at'); }
  });
  ...
  it('consumes snapshot bytes and logical paths only — never precomputed parse results', () => {
    ...
    expect(readYamlFile({ logicalPath: 'event.yaml', snapshot: mislabeled, schema: eventFileSchema })?.event).toBe('E1');
    ...
    expect(() => readYamlFile({ logicalPath: 'event.yaml', snapshot: brokenBytes, schema: eventFileSchema })).toThrow(ConfigError);
    ...
    expect(readYamlFile({ logicalPath: 'event.yaml', snapshot: again, schema: eventFileSchema })).toEqual(
  ...
  it('readYamlFilesInDir reads logical paths and bytes from the snapshot', () => {
    const source = snapshot({
      'definitions/characters/a.yaml': 'id: a\nname: A\ntype: person\ndescription: a\ninitialState: {}\ntraits: []\n',
      'definitions/characters/b.yaml': 'id: b\nname: B\ntype: person\ndescription: b\ninitialState: {}\ntraits: []\n',
