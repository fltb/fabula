...
export class PersistenceWorkerClient {
  ...
  request<O extends PersistenceOperation>(operation: O, payload: PersistencePayloads[O], signal?: AbortSignal): Promise<PersistenceResults[O]> {
    ...
    if (signal?.aborted) return Promise.reject({ code: 'ABORTED', message: 'Persistence task aborted before its next task boundary', retryable: false });
    return new Promise((resolve, reject) => {
      const abort = (): void => { if (this.#pending.delete(correlationId)) reject({ code: 'ABORTED', message: 'Persistence task aborted before its next task boundary', retryable: false }); };
  ...
  dispose(): void {
    ...
    for (const pending of this.#pending.values()) pending.reject(serializePersistenceError({ code: 'CLIENT_DISPOSED', message: 'Persistence client disposed' }));
    this.#pending.clear();
  }
}
