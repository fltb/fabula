  ...
  .object({
    ...
    capabilities: z.array(capabilityEntrySchema).superRefine((entries, context) => {
      ...
      for (const [index, entry] of entries.entries()) {
        if (ids.has(entry.id)) {
          context.addIssue({ code: z.ZodIssueCode.custom, path: [index, 'id'], message: 'Duplicate capability id' });
        }
        ids.add(entry.id);
      }
    }),
  ...
  .object({
    ...
    entries: z.array(
      z.discriminatedUnion('kind', [
        z.object({ eventId: z.string().min(1), kind: z.literal('generated'), runHash: z.string().min(1) }).strict(),
        z.object({
          eventId: z.string().min(1),
          ...
          overlap: z.object({ start: z.number().int().nonnegative(), end: z.number().int().positive(), hash: z.string().min(1) }).strict(),
        }).strict(),
      ]),
    ),
  })
...
export const responseReferenceSchema = z
  ...
  .superRefine((value, context) => {
    if (value.analysis.eventId !== value.metadata.eventId) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ['analysis', 'eventId'], message: 'Reference event IDs must match' });
    }
...
export const liveSmokeRecordSchema = z.object({
  version: schemaVersion,
  provider: z.string().min(1),
  model: z.string().min(1),
  seed: z.number().int(),
