...
function makeSnapshot(entries: Record<string, string>): ProjectSourceSnapshotV1 {
  ...
  return { version: 1, documents, sourceHash: hash(documents.map((d) => `${d.logicalPath}\0${d.content}`).join('')) };
}
...
describe('loadCanonicalProject — canonical kernel', () => {
  it('caches by immutable sourceHash while rebuilding mutable objects', () => {
    const source = snapshot(); const ir1 = loadCanonicalProject(source); const ir2 = loadCanonicalProject(source);
    expect(ir2.sourceHash).toBe(ir1.sourceHash); expect(ir2.data).not.toBe(ir1.data); expect(ir2.authoredEvents).not.toBe(ir1.authoredEvents); expect(ir2.registry).not.toBe(ir1.registry); expect(ir2.entityDeclarations).not.toBe(ir1.entityDeclarations);
  });
  it('reuses one runtime for byte-identical snapshots regardless of object identity', () => {
    ...
    expect(ir2.authoredEvents.map((event) => event.id)).toEqual(ir1.authoredEvents.map((event) => event.id));
    expect(ir2.runtimeEvents.map((event) => event.id)).toEqual(ir1.runtimeEvents.map((event) => event.id));
  ...
  it('rebuilds when one relevant authored source byte changes', () => {
    ...
    const entries = Object.fromEntries(base.documents.map((document) => [document.logicalPath, document.content]));
    entries['chapters/chapter_01/E0.yaml'] = entries['chapters/chapter_01/E0.yaml']!.replace('title: Encounter', 'title: Encounter!');
    ...
    expect(loadCanonicalProject(base).authoredEvents.find((event) => event.id === 'E0')!.title).toBe('Encounter');
  });
  it('never caches a failed mapping and ignores diagnostics for cache identity', () => {
    ...
    const badEntries = Object.fromEntries(valid.documents.map((document) => [document.logicalPath, document.content]));
    ...
    expect(loadCanonicalProject(valid).authoredEvents.find((event) => event.id === 'E0')!.title).toBe('Encounter');
    ...
    const noisy: ProjectSourceSnapshotV1 = {
      ...
      documents: valid.documents.map((document) => ({
        ...document,
        diagnostics: [{ code: 'noise', severity: 'warning', message: 'noise', logicalPath: document.logicalPath }],
      })),
    };
    expect(noisy.sourceHash).toBe(valid.sourceHash);
    expect(loadCanonicalProject(noisy).authoredEvents.find((event) => event.id === 'E0')!.title).toBe('Encounter');
  });
  it('never synthesizes genesis and stays package-private', () => {
    const ir = loadCanonicalProject(snapshot()); expect(ir.runtimeEvents.some((event) => event.id === 'system:genesis')).toBe(false); expect(ir.authoredEvents.some((event) => event.id === 'system:genesis')).toBe(false); expect('loadCanonicalProject' in entityIndex).toBe(false); expect('compileCanonicalRuntime' in entityIndex).toBe(false);
  });
  it('preserves authored targets and runtime boundaries', () => {
    const ir = loadCanonicalProject(snapshot()); const e0 = ir.authoredEvents.find((event) => event.id === 'E0'); expect(e0).toBeDefined(); expect(ir.runtimeEvents.some((event) => event.id === 'E0')).toBe(true); const runtime = compileCanonicalRuntime(ir); expect(runtime.boundaries.stateBeforeByEventId.has('E0')).toBe(true); expect(runtime.boundaries.stateAfterByEventId.has('E0')).toBe(true);
  });
  it('isolates cache entries and invalidates on changed sourceHash', () => {
    const irA = loadCanonicalProject(snapshot()); const irB = loadCanonicalProject(snapshot()); expect(irA).not.toBe(irB); expect(irA.data).not.toBe(irB.data); expect(irA.registry).not.toBe(irB.registry); const e0 = irA.authoredEvents.find((event) => event.id === 'E0')!; e0.title = 'mutated'; expect(irB.authoredEvents.find((event) => event.id === 'E0')!.title).not.toBe('mutated'); const changed = loadCanonicalProject(snapshot('Changed')); expect(changed.sourceHash).not.toBe(irA.sourceHash);
  });
});
