...
describe('project authoring runtime', () => {
    ...
    try {
      ...
      try {
        ...
        const revised = readFileSync(join(root, 'nova.yaml'), 'utf8').replace(
          /^title:.*$/m,
          'title: "Runtime Reconciled Fixture"',
        );
        ...
        expect(readFileSync(join(root, 'nova.yaml'), 'utf8')).toContain('Runtime Reconciled Fixture');
      } finally {
  ...
  it('removes a deleted accepted document from the catalog without overwriting working state', async () => {
    ...
    try {
      ...
      const snapshot = (entries: readonly { readonly logicalPath: string; readonly content: string }[]) =>
        buildSourceSnapshot(
          entries.map((entry) => ({
            version: 1 as const,
      ...
      await documents.seedFromAccepted(snapshot([{ logicalPath: 'nova.yaml', content: 'project: catalog\n' }]));
