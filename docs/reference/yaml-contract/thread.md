...
| `initialProgress` | `string` | **required** | — | Legacy progress label (e.g. `"0.00"`, `"0.15"`). Schema-validated as a plain string but currently **unvalidated and unused**: no decimal grammar is enforced, `buildInitialState` projects thread declarations to `{ id }` only, and replay baselines start from `status: "planned"` with empty goal/milestone state. |
...
- Declared threads become baseline `ThreadRuntimeState`s (`status: "planned"`, `currentRunId: "init-<threadId>"`) and root graph outputs `thread:<threadId>`.
