...
function seedCustomProject(storage: MemoryStorage, outputDir: string): void {
  ...
  storage.write(`${paths.sceneRevisionsDir}/E001/${revisionId}.json`, stableJson(envelope));
  ...
  storage.write(
    `${paths.transactionsDir}/tx-001.json`,
    stableJson({
  ...
  storage.write(
    `${paths.conflictsDir}/conflict-001.json`,
    stableJson({
      version: 1,
      operationId,
  ...
  storage.write(
    `${paths.tracesDir}/trace-001.json`,
    stableJson({
      operationId,
      events: [],
    }),
