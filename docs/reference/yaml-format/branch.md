...
- `targetEvent` 必须存在；目标的 `storyTime` 必须严格晚于 decision event。
...
`buildCausalEdges()` 在选择具体 `BranchPath` 后验证 explicit predecessor。unknown 或被该
branch 排除的 predecessor 是 `DagProviderError`。普通 Fact provider 也独立按每个 Fact 的
`validity.branches` 过滤，不能因为 event 本身可见而跨 lane 提供状态。

## 渲染与交付

- `renderNovel({ branchPath })` 只接受完整、按序的 leaf path；空 path、prefix、未知 choice
  或错误顺序在 cache、prompt、provider 之前失败。它只渲染该 route 的 authored EventFile；
  matching transition 只参与 state replay。
- `renderGameDialogueTree()` 以每个 event 的 representative leaf path 渲染所有节点一次，
  并验证所有共同节点的 `stateBefore` 字节一致。它拒绝 `renderSurface` 和带非 `main` label 的
  discourse ledger。
...
- API：`renderGameDialogueTree(opts)`；`RenderGameDialogueTreeOptions` 不接受 `eventId`、
  `branchPath` 或 `discourseBranch`。
...
不是 story `BranchPath`。选定 route 保留原有 discourse 规则；全树 render 因无法用一个
`discourseBranch` 代表所有 leaf，要求 ledger 只有 `main`。
