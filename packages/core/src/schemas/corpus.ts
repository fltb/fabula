...
export const informationActSchema = z
  .object({
...
export const ellipsisProvenanceSchema = z
  .object({
    sourceHash: z
      .string()
      .min(
        1,
        'Provenance sourceHash is required — every replay-changing Fact/effect MUST have atomic provenance',
      ),
    sourceRange: z
      .object({
...
export const narrativeEllipsisSchema = z
  .object({
    ...
    summary: z
      ...
      .optional()
      .describe(
        'Raw textual summary for source review/diagnostics only. NEVER enters logical prompt, produces Fact, causal edge, WorldState change, or DiscourseState change.',
      ),
  ...
  .strict(
    'NarrativeEllipsis schema does not accept unknown fields — POV, cast, sceneBrief, styleGuidance, narrativeOrder, narrationTime, targetAudience are forbidden',
  )
  .describe(
    'NarrativeEllipsis — non-renderable narrative gap with explicit constraints: no POV, no cast, no sceneBrief, no styleGuidance, no narrativeOrder, no narrationTime, no targetAudience, no sceneType, no discourseMode, no arcPosition.',
  );
...
export const narrativeNodeSchema = z
  .discriminatedUnion('kind', [narrativeEventSchema, narrativeEllipsisSchema])
  .describe(
