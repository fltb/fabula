...
const INVITE_ROLE_OPTIONS = PROJECT_ACCESS_ROLES.map(
  (role) => [role, role[0].toUpperCase() + role.slice(1)] as const,
);
...
export function AccessDevicesPage(props: AccessDevicesPageProps) {
  const [devices, setDevices] = createSignal<readonly WorkbenchDeviceSafeViewV1[]>(
    props.devices ?? [],
  );
  ...
  const projectOptions = () =>
    props.overview?.setup.projects.map(({ projectId: id, displayName }) => ({
      projectId: id,
      displayName,
    })) ?? [];
  ...
  const refreshDevices = () => {
    if (!props.client || props.authorization === 'user' || props.authorization === 'unauthorized')
      return;
    void props.client
      .listDevices()
  ...
  const createInvite = () => {
    void run(async () => {
      ...
      if (!selectedProjectId)
        throw new Error('A project must be selected before creating an invite.');
      ...
      setMessage(
        'Invite created. Only the safe project, role, expiry, and consumption state are shown.',
      );
    });
  };
  ...
  const issuePairing = () => {
    void run(async () => {
      const selectedProjectId = projectId().trim();
      if (!selectedProjectId)
        throw new Error('A project must be selected before issuing a device pairing.');
  ...
  const claimDevice = () => {
    void run(async () => {
      if (!pairingCode() || !deviceLabel().trim())
        throw new Error('Pairing code and device label are required.');
      ...
      setDevices((current) => [
        ...current.filter((device) => device.deviceId !== response.device.deviceId),
        response.device,
      ]);
      setDeviceLabel('');
  ...
  const copyCredential = async () => {
    if (!credential() || typeof navigator.clipboard?.writeText !== 'function') {
      setCredentialCopied(false);
      setMessage(
        'Clipboard access is unavailable; the credential remains visible only in this page.',
      );
      return;
    }
    try {
  ...
  return (
    <div class="grid gap-[var(--wb-space-6)]" data-testid="admin-access-devices-page">
      ...
      <Show when={props.authorization === 'user' || props.authorization === 'unauthorized'}>
        <section class={`${PANEL} border-[var(--wb-error-border)]`} role="alert">
          ...
          <p class="mt-[var(--wb-space-2)] text-sm leading-6 text-[var(--wb-ink-soft)]">
            This view is read-only. No invite, pairing, claim, or revoke mutation was sent.
          </p>
        </section>
      </Show>
      ...
      <section class="grid gap-[var(--wb-space-6)] lg:grid-cols-2">
        <section class={PANEL} aria-labelledby="invite-heading">
          ...
          <p class="mt-[var(--wb-space-2)] text-xs leading-5 text-[var(--wb-muted)]">
            Select a project and canonical access role. No redemption secret is returned to this
            page.
          </p>
          <form
          ...
          >
            ...
            >
              <Combobox.Label class="text-xs font-bold uppercase tracking-[0.06em] text-[var(--wb-muted)]">
                Project scope
              </Combobox.Label>
              <Combobox.Control class={`${INPUT} mt-2`}>
                <Combobox.Input required />
                <Combobox.Trigger aria-label="Choose invite project">⌄</Combobox.Trigger>
              </Combobox.Control>
              <Combobox.Portal>
            ...
            <SelectField
              label="Access role"
              value={inviteRole()}
              onChange={(next) => setInviteRole(next as ProjectAccessRole)}
              disabled={!authorized() || busy()}
            ...
            />
            <button class={BUTTON} type="submit" disabled={!authorized() || busy() || !projectId()}>
          ...
          <Show when={invite()}>
            {(created) => (
              <div
                class="mt-[var(--wb-space-5)] rounded-[var(--wb-radius-sm)] border border-[var(--wb-ready-border)] bg-[var(--wb-ready-surface)] p-[var(--wb-space-4)]"
                aria-live="polite"
              >
                <p class="font-semibold text-[var(--wb-success)]">Invite created</p>
                <dl class="mt-3 grid gap-2 text-xs text-[var(--wb-ink-soft)]">
                  <SafeField label="Invite id" value={created().inviteId} />
                  <SafeField label="Project" value={created().projectId ?? 'unknown'} />
                  <SafeField label="Role" value={created().role} />
                  <SafeField label="Expires" value={created().expiresAt} />
                </dl>
              </div>
            )}
          </Show>
        ...
        <section class={PANEL} aria-labelledby="pairing-heading">
          ...
          <p class="mt-[var(--wb-space-2)] text-xs leading-5 text-[var(--wb-muted)]">
            Issue a short-lived code, then claim it with a label and least scopes. The opaque
          ...
          <div class="mt-[var(--wb-space-5)] grid gap-[var(--wb-space-4)]">
            <button
              ...
              disabled={!authorized() || busy()}
            >
              {busy() ? 'Working…' : 'Issue pairing code'}
            </button>
            <Show when={pairingCode()}>
              <div class="rounded-[var(--wb-radius-sm)] border border-[var(--wb-loading-border)] bg-[var(--wb-loading-surface)] p-[var(--wb-space-4)]">
                <p class="text-xs font-bold uppercase tracking-[0.06em] text-[var(--wb-warning)]">
                  One-time pairing code
                </p>
                <code class="mt-2 block break-all text-lg font-semibold text-[var(--wb-ink)]">
                  {pairingCode()}
                </code>
                <p class="mt-2 text-xs text-[var(--wb-muted)]">Expires {pairingExpiresAt()}</p>
              </div>
            </Show>
            <Field label="Device label" id="device-label">
              <input
                id="device-label"
                class={INPUT}
                value={deviceLabel()}
                ...
                autocomplete="off"
                disabled={!authorized() || busy() || !pairingCode()}
              />
            </Field>
            <SelectField
              label="Credential lifetime"
              value={deviceTtl()}
              onChange={setDeviceTtl}
              disabled={!authorized() || busy() || !pairingCode()}
              options={[
                ['86400000', '24 hours'],
                ['2592000000', '30 days'],
            ...
            <fieldset
              ...
              disabled={!authorized() || busy() || !pairingCode()}
            >
              <legend class="text-xs font-bold uppercase tracking-[0.06em] text-[var(--wb-muted)]">
                Scopes
              </legend>
              <For each={SCOPES}>
                {(scope) => (
                  ...
                  >
                    <Checkbox.Input />
                    <Checkbox.Control class="grid h-5 w-5 place-items-center rounded border border-[var(--wb-border-strong)] bg-[var(--wb-surface)] data-[checked]:border-[var(--wb-accent)] data-[checked]:bg-[var(--wb-accent-wash)]">
                      <Checkbox.Indicator class="text-sm text-[var(--wb-accent-deep)]">
                        ✓
                      </Checkbox.Indicator>
                    </Checkbox.Control>
                    <Checkbox.Label class="text-sm text-[var(--wb-ink-soft)]">
                      {scope}
                    </Checkbox.Label>
                  </Checkbox>
                )}
            ...
            <button
              ...
              type="button"
              onClick={claimDevice}
              disabled={!authorized() || busy() || !pairingCode() || !deviceLabel().trim()}
            >
              Claim device
            </button>
            <Show when={credential()}>
              <div
                class="rounded-[var(--wb-radius-sm)] border border-[var(--wb-error-border)] bg-[var(--wb-error-surface)] p-[var(--wb-space-4)]"
                aria-live="assertive"
              >
                <p class="text-xs font-bold uppercase tracking-[0.06em] text-[var(--wb-danger)]">
                  Save this credential now — shown once
                </p>
                <code class="mt-2 block max-h-28 overflow-auto break-all text-sm text-[var(--wb-ink)]">
                  {credential()}
                </code>
                <div class="mt-3 flex flex-wrap gap-2">
                  ...
                  >
                    {credentialCopied() ? 'Copied' : 'Copy once'}
                  </button>
                  <button class={DANGER_BUTTON} type="button" onClick={() => setCredential('')}>
                    Clear credential
                  </button>
                </div>
              </div>
      ...
      <section class={PANEL} aria-labelledby="device-list-heading">
        <div class="flex flex-wrap items-end justify-between gap-[var(--wb-space-3)]">
          <div>
            <p class="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--wb-muted)]">
              Safe device registry
            </p>
            <h3
              id="device-list-heading"
              class="mt-[var(--wb-space-1)] text-lg font-semibold text-[var(--wb-ink)]"
            >
              Paired MCP devices
            </h3>
        ...
        <Show
          ...
          fallback={
            <p class="mt-[var(--wb-space-4)] text-sm leading-6 text-[var(--wb-muted)]">
              No devices are paired. Device credentials never appear in this list.
            </p>
          }
        >
          <ul class="mt-[var(--wb-space-4)] grid gap-[var(--wb-space-3)]">
            <For each={devices()}>
              {(device) => (
                <li
                  class={`${PANEL} flex flex-wrap items-center justify-between gap-[var(--wb-space-4)]`}
                >
                  <div class="grid gap-1">
                    <span class="font-semibold text-[var(--wb-ink)]">MCP device</span>
                    <span class="text-xs text-[var(--wb-muted)]">
                      {device.deviceId} · {device.scopes.join(', ')} · expires {device.expiresAt}
                    </span>
                    <Show when={device.revokedAt}>
                      <span class="text-xs font-semibold text-[var(--wb-danger)]">
                  ...
                  <button
                    ...
                    disabled={!authorized() || busy() || device.revokedAt !== null}
                  >
                    Revoke device
                  </button>
                </li>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          if (!open) setConfirmDeviceId(null);
        }}
      >
        <AlertDialog.Portal>
          ...
          <AlertDialog.Content class="fixed left-1/2 top-1/2 z-50 grid w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 gap-[var(--wb-space-4)] rounded-[var(--wb-radius-lg)] border border-[var(--wb-border-strong)] bg-[var(--wb-surface)] p-[var(--wb-space-6)] shadow-[var(--wb-shadow-drawer)]">
            <AlertDialog.Title class="font-display text-xl text-[var(--wb-ink)]">
              Revoke this MCP device?
            </AlertDialog.Title>
            <AlertDialog.Description class="text-sm leading-6 text-[var(--wb-muted)]">
              The Host will reject this device credential immediately. Other devices and project
