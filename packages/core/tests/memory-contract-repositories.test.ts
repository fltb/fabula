...
describe('memory semantic repositories', () => {
  it('isolates JSON-safe execution records and enforces optimistic CAS', async () => {
    ...
    const first = await repository.compareAndSwapAcceptedScene({
      projectId: 'project',
      eventId: 'event',
    ...
    ).toEqual({ kind: 'conflict', expectedVersion: null, actualVersion: 1 });
    const second = await repository.compareAndSwapAcceptedScene({
      projectId: 'project',
      eventId: 'event',
      expectedVersion: 1,
      value: { ...scene(), prose: 'Revised.' },
    });
  ...
  it('never resolves an accepted artifact from the render cache', async () => {
    ...
    await cache.put({
      key,
      record: { version: 1, key, recordHash: 'record', output: { prose: 'derived' } },
    });
    expect(await cache.get({ key })).not.toBeNull();
    expect(
      await execution.resolveAcceptedArtifact({ projectId: 'project', eventId: 'event' }),
    ).toBeNull();
  ...
  it('appends ordered state events with version conflicts and sequence reads', async () => {
    ...
    expect(await repository.append({ key, expectedVersion: 0, events: [event] })).toMatchObject({
      kind: 'appended',
      version: 1,
    });
    expect(
      await repository.append({
        key,
        expectedVersion: 0,
        events: [{ ...event, eventId: 'e2', sequence: 2 }],
      }),
    ...
    expect(
      (await repository.read({ key, fromSequence: 2 })).events.map(({ eventId }) => eventId),
    ).toEqual(['e2']);
  });
  ...
  it('selects nearest matching valid snapshot and safely returns null when absent', async () => {
    ...
    await repository.save({
      snapshot: { ...base, sequence: 5, schema: 'other' },
      expectedVersion: 1,
    });
    expect(
      await repository.readNearestValid({
        key,
        atOrBeforeSequence: 4,
        schema: 'world',
        schemaVersion: 1,
      }),
