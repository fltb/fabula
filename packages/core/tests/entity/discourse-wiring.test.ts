...
const EMPTY_STATE: WorldState = { entities: {}, relationships: {}, knowledge: {}, threads: {}, rules: {}, facts: [] };
const hash = (value: string) => createHash('sha256').update(value).digest('hex');
const source = (): ProjectSourceSnapshotV1 => {
  const entries: Record<string, string> = {
    'nova.yaml': 'project: discourse-test\ntitle: Discourse Test\nauthor: Test\ndefaultLanguage: en\ndefaultModel: mock-pass2\n',
...
describe('discourse wiring — immutable source snapshot', () => {
  ...
  it('compiles a discourse context for the authored event', () => {
    const data = new EntityMapper(source()).loadProject(); const events = new EntityMapper(source()).loadAllEvents(data); const contexts = compileDiscourseBoundaries(events, data.discourseLedger, data.narratorAssertions, data.narratorProfiles, 'main'); const event = events[0]; const registry = new InMemoryEntityRegistry(); registry.load(data); const pkg = new ContextCompiler().compile(event, EMPTY_STATE, registry, { narratorProfiles: data.narratorProfiles, discourseContext: contexts[event.id] });
    expect(pkg.narratorProfile?.id).toBe('narrator_wo'); expect(pkg.discourseReplayError).toBeUndefined();
  });
});
