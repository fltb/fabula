...
export class CharacterStateValidator implements Validator {
  ...
  validatePost(input: PostRenderInput): ValidationIssue[] {
    ...
    for (const [entityId, entityState] of Object.entries(input.worldState.entities)) {
      ...
      const actionVerbs =
        /\b(spoke|said|walked|ran|thought|felt|decided|nodded|smiled|frowned|looked|grabbed|stepped|sat|stood|replied|asked|whispered|shouted|moved)\b/i;
      ...
      for (const sentence of sentences) {
        if (namePat.test(sentence) && actionVerbs.test(sentence)) {
          ...
          const isFlashback =
            /\b(remembered|recalled|flashback|memory|thought back|imagined)\b/i.test(sentence);
          if (!isFlashback) {
            issues.push(
              makeIssue(
                this.name,
